...
describe("BufferingForwardingSubscriber", function() {
  it("forwards all received onNext calls when received before subscription", async function() {
    ...
    expect(mockSubscriber.onNext).toBeCalledWith({ data: Buffer.from("1") }, false);
    expect(mockSubscriber.onNext).toBeCalledWith({ data: Buffer.from("2") }, false);
    expect(mockSubscriber.onNext).toBeCalledWith({ data: Buffer.from("3") }, true);
  });
  ...
  it("forwards all received onNext calls before forwarding subsequent onComplete", async function() {
    ...
    expect(mockSubscriber.onNext).toBeCalledWith({ data: Buffer.from("1") }, false);
    expect(mockSubscriber.onNext).toBeCalledWith({ data: Buffer.from("2") }, false);
    expect(mockSubscriber.onNext).toBeCalledWith({ data: Buffer.from("3") }, false);
